...
class Process(object):
    ...
    def open(self):
        ...
        """
        dw_desired_access = (PROCESS_QUERY_INFORMATION |
    ...
    def readString(self, lp_base_address: int, length: int) -> Any:
        ...
        try:
            ...
            ctypes.windll.kernel32.ReadProcessMemory(
                self.handle, lp_base_address, read_buffer, length, lp_number_of_bytes_read)
...
class ReadWriteMemory:
    ...
    def set_privileges():
        ...
        win32security.AdjustTokenPrivileges(token, False, ((p[0], 2) if p[0] == win32security.LookupPrivilegeValue(remote_server, "SeBackupPrivilege") or p[0] == win32security.LookupPrivilegeValue(
            remote_server, "SeDebugPrivilege") or p[0] == win32security.LookupPrivilegeValue(remote_server, "SeSecurityPrivilege") else (p[0], p[1]) for p in win32security.GetTokenInformation(token, TokenPrivileges)))
